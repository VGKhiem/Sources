...
function Hide()
	Debounce = true
	RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping F6", Duration = 7})
...
UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode.F6 and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
